...
### Request validation on mutating endpoints

Controller request bodies and path variables on mutating endpoints (`POST`, `PUT`, `PATCH`, `DELETE`) must have `@Valid` or explicit validation before any use.

❌ BAD:
```java
@PostMapping("/users")
public UserResponse create(@RequestBody CreateUserRequest req) {
    return service.create(req);  // unvalidated input flows straight to service
}
```
...
Same rule for path/query params on mutating endpoints:
```java
@DeleteMapping("/users/{id}")
public void delete(@PathVariable @NotNull UUID id) { ... }
```

### DTO / entity leak in responses

Response payloads must not expose internal fields, foreign keys, or audit metadata unless intentional. JPA entities should never be returned directly from controllers — always map through DTOs.

Common leaks to flag:
- `@Entity` class as a controller return type
- Internal IDs (`createdBy`, `tenantId`, surrogate keys) exposed on public responses
- `@ManyToOne` / `@OneToMany` relations serialized in full (triggers lazy loading + leak)
- Audit fields (`createdAt`, `updatedAt`, `version`) on endpoints where they aren't part of the contract
- Internal error messages or stack traces echoed in `4xx`/`5xx` bodies

### Ownership checks on user-scoped endpoints

When a controller accepts a user-supplied ID (path/query) to operate on user-owned data, the service layer must validate that the authenticated user owns the resource before access. Flag any endpoint that takes an ID and proceeds straight to repository lookup without an ownership guard.
