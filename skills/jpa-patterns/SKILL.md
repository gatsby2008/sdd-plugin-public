...
### JPQL & native query parameterization

JPQL and native queries must use named parameters — never string concatenation built from user input.
...
Flag any query construction using `String.format`, `+` concatenation, or `StringBuilder` building JPQL/SQL from inputs reachable from a controller or external payload.

### Repository scoping

Repository methods that return user-scoped data must filter by the authenticated user's identifier. Flag any method that returns all rows of a sensitive entity without a `WHERE` clause tied to user context.

---

## Related Skills

- `code-review` - Stack-aware code quality and security review
