...
> Applied by `sdd:java:security-reviewer` when this pack is loaded.

### Never log PII at any level

Personally identifiable information must not appear in log messages — not even at DEBUG, not even in "temporary" debugging statements that survive a PR. PII includes:

- Names, emails, phone numbers, addresses
- Government IDs (SSN, DNI, passport), account numbers
- Dates of birth, biometric data
- Full credit card / banking details (PAN, CVV, IBAN)
- Authentication tokens, session IDs, password hashes

Log a correlation identifier (request ID, `applicationUuid`, hashed user ID) instead. If a domain object you must log contains PII, redact it explicitly before logging:

❌ BAD:
```java
log.info("Created user {}", user);            // toString likely leaks email/phone
log.debug("Verifying SMS for {}", phoneNumber);
log.info("Login attempt: {} / {}", username, password);  // credential leak
```

✅ GOOD:
```java
log.info("Created user applicationUuid={}", user.getApplicationUuid());
log.debug("Verifying SMS requestId={}", requestId);
log.info("Login attempt requestId={}", requestId);
```

### Feign client logging hygiene

`feign.client.config.*.loggerLevel` must be `NONE` or `BASIC` in production configs. `HEADERS` logs `Authorization` and other auth tokens; `FULL` logs request/response bodies, which routinely contain PII.
...
Flag any production config (`application.yml`, `application-prod.yml`, environment overrides) that sets `loggerLevel: HEADERS` or `loggerLevel: FULL`.

### No secrets or credentials in any log statement

Never log API keys, passwords, tokens, private keys, or connection strings — including in error messages or stack traces that bubble up from a third-party SDK. If a downstream library logs sensitive payloads by default, disable or filter that logger explicitly.

### SQS payload logging

When logging SQS messages, log only the message ID and correlation identifiers — never the full payload, which may contain PII or business-sensitive data.
